...
# 概要
まずは概要の説明です。私は大きく分けて2つの文にまとめました。
説明はスライドに載っている通りにはなります。
まず、ViTでは画像パッチを同じサイズに区切っているのに対して大小異なるサイズのパッチに区切ることで異なるスケールの情報を取り扱うことができるCrossViTを提案しました。
...
# 新規性・結果・新たな知見
次に新規性について話していきます。
この論文からわかった新規性は、小さなパッチと大きなパッチを別々のトランスフォーマーに入力することです。次のスライドで画像を用いて説明します。
...
この二つのブランチ間でトークンを融合するために囲われている赤い部分があると思うのですが、こちらはクロスアテンションモジュールを導入しています。このモジュールは、各ブランチから選択されたトークンをクエリとして使用し、他のブランチからすべてのトークンとアテンションを計算しています。
...
CrossViTは、EfficientNetやResNetなどの効率的なCNNモデルと比較して競合することがわかりました。実験の結果表を見ると、ResNetは、CNNモデルの中でも最先端のものであり、画像分類のタスクにおいて高い性能を発揮していることもわかると思います。そのようなモデルと比較しても劣らないかそれ以上の精度を達成したことはCrossViTの精度の良さがわかると思います。
...
最後にクロスアテンションによる精度は、大小の異なるブランチで学習を行っているためバランスを取ることが可能となりました。
