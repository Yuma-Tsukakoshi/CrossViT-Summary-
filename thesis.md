...
# 簡潔1スライド用
## 概要
画像分類のためのトランスフォーマーモデルの性能と効率性を向上させることができるようになった。トランスフォーマーモデルは、自然言語処理の分野で大きな成功を収めているが、画像処理の分野ではまだ発展途上です。この論文は、画像を小さく切ってそれぞれに番号をつけるという手法を用いて、異なるサイズの画像パッチを組み合わせて強力な画像特徴を生成することができることを示しました。また、計算量やメモリ量を削減するために、各枝の単一のトークンをクエリとして使用して他の枝と情報を交換するというクロスアテンションというモジュールを開発した。このモジュールは、計算とメモリの複雑さの両方に対して線形時間しか必要とせず、それ以外では二次時間が必要である。この論文は、画像分類におけるトランスフォーマーモデルの可能性と応用範囲を広げることがが可能となった。
...
<計算量を削減するためにクロスアテンションに基づくシンプルで効果的なトークン融合モジュールを開発しています。このモジュールは、各ブランチの単一のトークンをクエリとして使用して他のブランチと情報を交換します。)
